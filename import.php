...
class convertToOwncloud
{
    ...
    private function writeDb($data, $card, $book)
    {
        ...
        $mysqli2->query("INSERT INTO oc_addressbookchanges (addressbookid ,synctoken,uri ,operation) VALUES (".$book.", ".$this->getLastToken('oc_addressbookchanges').", '" . $uri . "', 3)") OR die(mysqli_error($mysqli2));
        $mysqli2->query("INSERT INTO oc_addressbookchanges (addressbookid ,synctoken,uri ,operation) VALUES (".$book.", ".$this->getLastToken('oc_addressbookchanges').", '" . $uri . "', 1)") OR die(mysqli_error($mysqli2));
        $mysqli2->close();
    }
    ...
    private function deleteDb($data, $book) {
        ...
        $uri = $data['uid'] . ".vcf";
        $mysqli2->query("DELETE FROM oc_cards WHERE uri = '" . $uri . "' AND addressbookid = 1");
        $mysqli2->query("INSERT INTO oc_addressbookchanges (addressbookid ,synctoken,uri ,operation) VALUES (".$book.", ".$this->getLastToken('oc_addressbookchanges').", '" . $uri . "', 2)") OR die(mysqli_error($mysqli2));
        $mysqli2->close();
    }
    ...
    {
        $mysqli2 = new mysqli($this->host_carddav, $this->db_user_carddav, $this->db_pass_carddav, $this->db_name_carddav);
        $mysqli2->query("UPDATE `oc_addressbooks` SET `synctoken` = " . $this->getLastToken('oc_addressbooks') . " WHERE `id` =1 LIMIT 1 ");
    }
