...
class convertToOwncloud
{
    ...
    private function writeDb($data, $card, $book)
    {
        ...
        $uri = $data['uid'] . ".vcf";
        $mysqli2->query("DELETE FROM oc_contacts_cards WHERE uri = '" . $uri . "' AND addressbookid = 1");
    ...
    private function makeGroups($group_choice)
    {
        ...
        foreach ($this->groups as $group => $group_name) {
            $mysqli2->query("DELETE FROM oc_contacts_cards WHERE fullname = '" . $group_name['name'] . "' AND addressbookid = 1");
